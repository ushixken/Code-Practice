...
*/
;(function () {
  const plannerConfig = window.PLANNER_CONFIG
  const NODE_TYPES = plannerConfig.nodeTypes
  const STATUS_ORDER = plannerConfig.statusOrder
  const STATUS_LABEL = plannerConfig.statusLabels
  const CANVAS_W = plannerConfig.canvas.width
  const CANVAS_H = plannerConfig.canvas.height
  ...
  function buildShell() {
    ...
    templatePicker.addEventListener("change", () => {
      ...
      templateModal.querySelector(".planner-template-modal-name").textContent =
        templatePicker.options[templatePicker.selectedIndex].text
    ...
    redoBtn.title =
      "Redo the last undone planning change (Ctrl/Cmd + Shift + Z or Ctrl + Y)"
    ...
    templateModal.addEventListener("click", (e) => {
      if (
        ...
        e.target.dataset.templateAction === "cancel"
      ) {
    ...
    arrowLabelModal.addEventListener("click", (e) => {
      ...
      if (action === "save") {
        ...
        if (edge) {
          const label = arrowLabelModal
            .querySelector(".planner-arrow-label-input")
            .value.trim()
          if (label !== edge.label) {
            saveHistory()
            edge.label = label
    ...
    arrowLabelModal
      ...
      .addEventListener("keydown", (e) => {
        if (e.key === "Enter")
          arrowLabelModal
            .querySelector('[data-arrow-label-action="save"]')
            .click()
  ...
  function templateDefinition(kind, baseX, baseY) {
    const templates = {
      ...
      sample: [
        {
          type: "problem",
          text: "Problem: app saves duplicate items",
          x: baseX + 10,
          y: baseY,
        },
        {
          type: "note",
          text: "Known: it happens after a slow connection",
          x: baseX + 300,
          y: baseY,
        },
        {
          type: "task",
          text: "Make a small reproduction",
          x: baseX + 10,
          y: baseY + 120,
        },
        {
          type: "io",
          text: "Record input and output",
          x: baseX + 10,
          y: baseY + 230,
        },
        {
          type: "decision",
          text: "Can I reproduce it?",
          x: baseX + 10,
          y: baseY + 340,
        },
        {
          type: "blocked",
          text: "Ask for logs if it cannot reproduce",
          x: baseX + 310,
          y: baseY + 340,
        },
        {
          type: "function",
          text: "Inspect saveItem()",
          x: baseX + 10,
          y: baseY + 480,
        },
        {
          type: "loop",
          text: "Repeat with one changed variable",
          x: baseX + 10,
          y: baseY + 590,
        },
        {
          type: "process",
          text: "Apply the smallest safe fix",
          x: baseX + 10,
          y: baseY + 700,
        },
        {
          type: "terminal",
          text: "End: verify the fix",
          x: baseX + 40,
          y: baseY + 810,
        },
      ],
      bug: [
        ...
        {
          type: "note",
          text: "Gather error details / expected behavior",
          x: baseX + 300,
          y: baseY + 220,
        },
        {
          type: "process",
          text: "Trace the relevant code",
          x: baseX + 10,
          y: baseY + 360,
        },
        {
          type: "decision",
          text: "Is the cause clear?",
          x: baseX + 10,
          y: baseY + 470,
        },
        {
          type: "note",
          text: "Collect evidence and revise hypothesis",
          x: baseX + 300,
          y: baseY + 470,
        },
        {
          type: "task",
          text: "Try the smallest safe fix",
          x: baseX + 10,
          y: baseY + 610,
        },
        {
      ...
      feature: [
        ...
        {
          type: "task",
          text: "Break it into small tasks",
          x: baseX + 10,
          y: baseY + 210,
        },
        {
          type: "process",
          text: "Build the smallest useful piece",
          x: baseX + 10,
          y: baseY + 320,
        },
        {
          type: "decision",
          text: "Does it behave as planned?",
          x: baseX + 10,
          y: baseY + 430,
        },
        {
        ...
        {
          type: "decision",
          text: "More small pieces?",
          x: baseX + 10,
          y: baseY + 570,
        },
        {
          type: "process",
          text: "Polish and review",
          x: baseX + 10,
          y: baseY + 710,
        },
      ...
      refactor: [
        {
          type: "problem",
          text: "Name the pain point",
          x: baseX + 10,
          y: baseY,
        },
        {
          type: "note",
          text: "Define a safe boundary",
          x: baseX + 10,
          y: baseY + 110,
        },
        {
          type: "decision",
          text: "Is behavior protected?",
          x: baseX + 10,
          y: baseY + 220,
        },
        {
          type: "task",
          text: "Add a safety check",
          x: baseX + 300,
          y: baseY + 220,
        },
        {
          type: "process",
          text: "Make one small change",
          x: baseX + 10,
          y: baseY + 360,
        },
        {
          type: "decision",
          text: "Still behaves correctly?",
          x: baseX + 10,
          y: baseY + 470,
        },
        {
          type: "note",
          text: "Undo / reduce the change",
          x: baseX + 300,
          y: baseY + 470,
        },
        {
          type: "decision",
          text: "More cleanup needed?",
          x: baseX + 10,
          y: baseY + 610,
        },
    ...
    return {
      seq,
      connections: templateConnections[kind] || templateConnections.logic,
    }
  }
  ...
  function insertTemplate(kind = "logic", options = {}) {
    ...
    const link = (a, b, label) =>
      state.edges.push({ id: uid() + "e", from: a, to: b, label })
  ...
  function centerOnNodes(smooth) {
    ...
    if (!wrap) return
    const z = state.zoom || 1
    let targetX = CANVAS_ORIGIN_X,
      targetY = CANVAS_ORIGIN_Y
  ...
  function renderNode(inner, n) {
    ...
    if (state.edges.some((e) => e.to === n.id))
      inSocket.classList.add("has-link")
    ...
    if (state.edges.some((e) => e.from === n.id))
      outSocket.classList.add("has-link")
    ...
    el.addEventListener("click", (e) => {
      ...
      inner
        .querySelectorAll(".pnode.selected")
    ...
    el.addEventListener("mouseup", (e) => {
      const changesPlan =
        (linking.active &&
          ((linking.fromId && linking.fromId !== n.id) ||
            (linking.toId && linking.toId !== n.id))) ||
  ...
  function makeNodeDraggable(el, n) {
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      let x = Math.min(
        CANVAS_W - 40,
        Math.max(0, ox + (e.clientX - sx) / state.zoom),
      )
      let y = Math.min(
        CANVAS_H - 40,
        Math.max(0, oy + (e.clientY - sy) / state.zoom),
      )
      ...
      if (inner) {
        ...
        connectedIds.forEach((id) => {
          ...
          if (snappedX === null && Math.abs(centerX - otherCenter.x) <= SNAP_PX)
            snappedX = otherCenter.x
          if (snappedY === null && Math.abs(centerY - otherCenter.y) <= SNAP_PX)
  ...
  function setupPanZoom(wrap) {
      ...
      if (templatePlacement.kind) {
        ...
        if (inner) {
          ...
          updateTemplatePreview(
            (e.clientX - rect.left) / state.zoom,
            (e.clientY - rect.top) / state.zoom,
          )
        }
      }
      if (panning) {
    ...
    window.addEventListener("mouseup", () => {
      ...
      if (linking.active) {
        ...
        linking.toId = null
        document
          .querySelectorAll(".pnode.linking-source")
          .forEach((el) => el.classList.remove("linking-source"))
        render()
      }
      if (detaching.active) {
  ...
  function edgeAnchor(el, towardX, towardY, slotIndex, slotCount) {
    ...
    if (slotCount > 1) {
      const angle =
        Math.atan2(dy, dx) + (slotIndex - (slotCount - 1) / 2) * SLOT_ANGLE
      dx = Math.cos(angle)
      dy = Math.sin(angle)
    }
  ...
  function drawEdges(liveMouseEvent) {
    ...
    if (!svg || !inner) return
    svg
      .querySelectorAll(".pedge, .pedge-hit, .pedge-label, .pedge-live")
      .forEach((e) => e.remove())
    const z = state.zoom || 1
    ...
    state.edges.forEach((edge) => {
      ...
      const alignedVertically =
        !alignedHorizontally && Math.abs(fromCenter.x - toCenter.x) <= SNAP_PX
      ...
      drawBezier(svg, p1.x, p1.y, p2.x, p2.y, "pedge", edge.label, false, (e) =>
        startDetach(e, edge, p1, p2),
      )
    })
    ...
    if (linking.active && (linking.fromId || linking.toId) && liveMouseEvent) {
      ...
      if (linking.fromId) {
        const fromEl = inner.querySelector(
          `.pnode[data-id="${linking.fromId}"]`,
        )
        if (fromEl) {
    ...
    if (detaching.active && liveMouseEvent) {
      const fixedEl = inner.querySelector(
        `.pnode[data-id="${detaching.fixedId}"]`,
      )
      if (fixedEl) {
        const isOutputEnd = detaching.movingEnd === "to" // fixed end is the source (tail)
        const group = isOutputEnd
          ? outGroups.get(detaching.fixedId) || []
          : inGroups.get(detaching.fixedId) || []
        const innerRect = inner.getBoundingClientRect()
        let mouseP = {
          x: (liveMouseEvent.clientX - innerRect.left) / z,
          y: (liveMouseEvent.clientY - innerRect.top) / z,
  ...
  function drawBezier(svg, x1, y1, x2, y2, cls, label, isLive, onGrab) {
    ...
    help.textContent =
      "Drag an end to reconnect · Shift-click to reverse · Alt-click to label"
  ...
  function setupNodeDrop(wrap) {
    ...
    wrap.addEventListener("dragover", (e) => {
      if (!e.dataTransfer.types.includes("application/x-planner-node-type"))
        return
      e.preventDefault()
    ...
    wrap.addEventListener("drop", (e) => {
      ...
      const x = Math.min(
        CANVAS_W - 40,
        Math.max(0, (e.clientX - innerRect.left) / z - 70),
      )
      const y = Math.min(
        CANVAS_H - 40,
        Math.max(0, (e.clientY - innerRect.top) / z - 30),
      )
